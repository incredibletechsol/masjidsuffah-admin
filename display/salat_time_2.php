...
echo "[";
if ($result = $db->query("SELECT * FROM salat_times where DATE_FORMAT(prayer_date, '%Y-%m-%d') = '" . $monthDate . "' limit 1")) {
...
if (isset($salatTimes)) {
    ...
    if ($result = $db->query("SELECT * FROM salat_times where DATE_FORMAT(prayer_date, '%Y-%m-%d') > '" . $monthDate . "'")) {
